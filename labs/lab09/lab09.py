...
with open('words_updated.txt', 'r') as infile:
...
# Get valid input from the user for k
...
while True:
    try:
        ...
        if 1 <= k <= 80:
            ...
            print("Please enter a value between 1 and 80.")
    except ValueError:
...
for word in words:
    ...
    if len(current_line.replace(' ', '')) + len(word) <= k:  \
        ...
        if current_line:
            ...
            # Write a new line and then the current line
            output_file.write('\n' + current_line)  # Write a newline before the current line
            current_line = word  # Start a new line with the current word

    # Don't forget to write the last line after the loop
if current_line:
        output_file.write('\n' + current_line)  # Write the last line to the file
