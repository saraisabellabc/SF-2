...
def detect_fall(velocity, time):
    ...
    for cursor_end in range(start + 1, len(velocity) - 1):
        if abs(velocity[cursor_end] - velocity[cursor_end + 1]) > 0.5 and np.sign(velocity[cursor_end]) != np.sign(velocity[cursor_end + 1]) or abs(velocity[cursor_end] - velocity[cursor_end + 1]) > 1:
            cursor_end += 1
            break
    end = cursor_end
...
#load file in
nom_file = "multiples_filters.csv"
...
for i in range(1, 11):
    ...
    filtered_multiple_file = f"filtered_mutltiple_file{i}.csv" #process of creating csv (pandas 2 end)
